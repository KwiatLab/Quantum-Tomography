...
def main():
    ...
    parser.add_argument("-i", "--eval", type=dir_path, nargs=1,
    ...
    parser.add_argument("-o", "--outPut", type=dir_path, nargs=1,
                        metavar="outPutFolder", default= ["../"], help="the full path to the folder where you want the output to be saved")
    ...
    parser.add_argument("-s", "--save", type=bool, nargs=1,
                        metavar="saveBool", default=True,
                        help=" Setting this to false will NOT SAVE your output data. Only do this if you just want the output to be printed in the command line")
    ...
    outputVals, outputNames = qKLib.displayOutput(rho, intensity, fval,t)
    if(save):
        ...
        if(pictures):
            ...
            FORREPLACE = FORREPLACE + '<img src="rhobarRreal.png" style="float: left;" width="550" height="auto">'
            FORREPLACE = FORREPLACE + '<img src="rhobarRimag.png" width="550" height="auto"><br>'
        ...
        for name in outputNames:
            data = outputVals[name]
            f += '<tr><td><div onmouseover="Tip( ' + name.replace(" ",
                                                                  "") + 'Tip)" onmouseout="hideTip()">' + name + '</td><td name= "' + name.replace(
                " ", "") + '_value">' + str(data[0]) + '</td>'
            if (len(data) > 1):
                if (data[1] == data[1]):
                    f += '<td> +/- %s</td></tr>' % str(data[1])
                else:
                    f += "<td></td></tr>"
            else:
                f += "<td></td></tr>"
        ...
        part = 5
        bell_times = 3
        if (t.conf['Bellstate'] != 0) & (t.err_n > 1):
            [bellerrs, bellmeans] = t.tomography_error_bell(rhon, part_init, part, bell_times, t.err_n)
            bell = t.websitebellsettings(rho, part_init, part, bell_times)
            resolution = (np.pi / 2) / (part_init * (part ** bell_times))
            ...
            if t.err_n == 1:
                bs += '<tr><td>s</td><td>' + str(round(bell[0], 5)) + '</td></tr>'
                bs += '<li>a = ' + r2d(bell[1]) + ' &deg;</li> \n'
                bs += '<li>a&prime;= ' + r2d(bell[2]) + ' &deg;</li> \n'
                bs += '<li>b = ' + r2d(bell[3]) + ' &deg;</li> \n'
                bs += '<li>b&prime;= ' + r2d(bell[4]) + ' &deg;</li> \n'
            else:
                bs += '<tr><td>s</td><td>' + str(round(bellmeans[0], 5)) + ' &deg;</td><td>+/- ' + str(
                    round(bellerrs[0], 5)) + '&deg;</td></tr> \n'
                bs += '<tr><td>a</td><td>' + r2d(bellmeans[1]) + ' &deg;</td><td>+/- ' + r2d(
                    bellerrs[1]) + '&deg;</td></tr> \n'
                bs += '<tr><td>a&prime;</td><td>' + r2d(bellmeans[2]) + ' &deg;</td><td>+/- ' + r2d(
                    bellerrs[2]) + '&deg;</td></tr> \n'
                bs += '<tr><td>b</td><td>' + r2d(bellmeans[3]) + ' &deg;</td><td>+/- ' + r2d(
                    bellerrs[3]) + '&deg;</td></tr> \n'
                bs += '<tr><td>b&prime;</td><td>' + r2d(bellmeans[4]) + ' &deg;</td><td>+/- ' + r2d(
                    bellerrs[4]) + '&deg;</td></tr> \n'
            bs += '<tr><td>resolution</td><td>' + str(resolution * 180 / np.pi) + '&deg;</td><td></tr></tr> \n'
            bs += '</td></tr></table>'
        bs = bs.replace('+/- nan&deg;', '')
        bs = bs.replace('nan &deg;', '')
