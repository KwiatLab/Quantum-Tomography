    ...
    for TomographyNumber in range(nStates):
        ...
        if Tomo_Object.getNumDetPerQubit() == 1:
            ...
            measurements_raw = tomo_input[
                :, np.arange(2**numQubits + 2 * numQubits + 1, 2**numQubits + 4 * numQubits + 1)
            ]
        crosstalk = Tomo_Object.conf["Crosstalk"]

        # NEW WAY
        measurements_densities = np.zeros(
            (tomo_input.shape[0], Tomo_Object.getNumCoinc(), 2**numQubits, 2**numQubits), dtype=complex
        )
        ...
        for i in range(len(tomo_input)):
            if test2Det:
                ...
                prob = np.real(prob)
                tomo_input[i, numQubits + 1] = np.random.binomial(numCounts, min(prob, 0.99999999))
        ...
        if testAccCorr and numQubits < 3:
            ...
            if test2Det:
                ...
                coinc = tomo_input[:, np.arange(2 * numQubits + 1, 2**numQubits + 2 * numQubits + 1)]
            ...
            if test2Det:
                ...
                tomo_input[:, np.arange(2 * numQubits + 1, 2**numQubits + 2 * numQubits + 1)] = coinc + acc
            else:
    ...
    for t in Tomographys:
        ...
        if saveData:
            ...
            dataRow["test_cross"] = np.any(
                Tomo_Object.conf["Crosstalk"] - np.eye(Tomo_Object.conf["Crosstalk"].shape[0]) > 1e-6
            )
