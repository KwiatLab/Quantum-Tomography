    ...
    for TomographyNumber in range(nStates):
        ...
        if isinstance(testCrossTalk, bool) and testCrossTalk:
            ...
            for i in range(2**numQubits):
                cTalkMat[:, i] = cTalkMat[:, i] / (
                    sum(cTalkMat[:, i] + 0 * np.random.random())
                )
        ...
        if Tomo_Object.getNumDetPerQubit() == 1:
            ...
            measurements_raw = tomo_input[
                :, np.arange(numQubits + 2, 3 * numQubits + 2)
            ]
        else:
            ...
            measurements_raw = tomo_input[
                :,
                np.arange(
                    2**numQubits + 2 * numQubits + 1, 2**numQubits + 4 * numQubits + 1
                ),
            ]
        ...
        for j in range(tomo_input.shape[0]):
            meas_basis_densities = (
                np.zeros([2**numQubits, 2**numQubits, 2**numQubits]) + 0j
            )
            meas_basis_pures = 1
        ...
        if isinstance(randomStateDist, str):
            if randomStateDist == "density":
                ...
                raise ValueError(
                    '"' + randomStateDist + '" is not a valid state distribution. '
            ...
            if abs(np.log2(randomStateDist.shape[0]) - numQubits) < 10**-6:
                ...
                raise ValueError(
                    '"'
                    + str(randomStateDist)
                    + '" does not have the right dimensions for '
                    + str(numQubits)
                    + " Qubits."
                )
        else:
            raise ValueError(
                '"' + str(randomStateDist) + '" is not a valid state distribution.'
            )

        # counts
        numCounts = int(
            np.random.randint(np.ceil(5 * 2**numQubits), np.floor(50 * 2**numQubits))
        )
        ...
        for i in range(len(tomo_input)):
            if test2Det:
                ...
                for j in range(0, 2 * numQubits):
                    prob[j] = np.trace(
                        np.matmul(measurements_densities[i, j], startingRho)
                    )
                    prob[j] = min(np.real(prob[j]), 0.99999999)
                prob = np.real(prob)
                tomo_input[i, 2 * numQubits + 1 : 2**numQubits + 2 * numQubits + 1] = (
                    np.random.multinomial(numCounts, prob)
                )
            else:
                prob = np.trace(np.matmul(measurements_densities[i, 0], startingRho))
                prob = np.real(prob)
                tomo_input[i, numQubits + 1] = np.random.binomial(
                    numCounts, min(prob, 0.99999999)
                )
        ...
        if testDrift:
            intensities = np.random.random(intensities.shape) * 2 + 0.25
            if test2Det:
                # tomo_input[:, np.arange(2*n_qubit+ 1, 2**n_qubit+ 2*n_qubit+ 1)]: coincidences
                coinc_range = np.arange(
                    2 * numQubits + 1, 2**numQubits + 2 * numQubits + 1
                )
                for k in range(tomo_input.shape[0]):
                    tomo_input[k, coinc_range] = (
                        intensities[k] * tomo_input[k, coinc_range]
                    ).real.astype(int)
            else:
        ...
        try:
            ...
            myDensity, inten, myfVal = Tomo_Object.StateTomography_Matrix(
                tomo_input, intensities, method=method
            )
            end_time = time.time()
            ...
            if (
                Fidelity_with_Original < 0.8
                and not testCrossTalk
                and np.average(Tomo_Object.getCoincidences()) > 10
                and method.upper() != "LINEAR"
            ):
                print("-----------------------------")
                print(
                    "Low Fidelity of "
                    + str(Fidelity_with_Original)
                    + ". Avg counts per basis = "
                    + str(np.average(Tomo_Object.getCoincidences()))
                )
        except:
            Original_Purity = -1
...
def saveRunsGeneral(
    numQubits,
    nStates,
    resultsFilePath="Results/results_GeneralData.csv",
    randomStateDist="density",
    errBounds=0,
    testAccCorr=False,
    test2Det=False,
    ...
    Tomographys = runTests(
        numQubits,
        nStates,
        randomStateDist=randomStateDist,
        errBounds=errBounds,
        testAccCorr=testAccCorr,
        test2Det=test2Det,
        testCrossTalk=testCrossTalk,
        testBell=testBell,
        testDrift=testDrift,
        method=method,
    )
    ...
    for t in Tomographys:
        ...
        if saveData:
            ...
            dataRow["test_cross"] = np.any(
                Tomo_Object.conf["Crosstalk"]
                - np.eye(Tomo_Object.conf["Crosstalk"].shape[0])
                > 1e-6
            )
...
def uniqueID(Tomo_Object):
    ...
    if np.any(
        Tomo_Object.conf["Crosstalk"] - np.eye(Tomo_Object.conf["Crosstalk"].shape[0])
        > 10**-6
    ):
        testCrossTalk = True
    else:
