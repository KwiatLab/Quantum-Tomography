...
if __name__ == "__main__":
    ...
    stabilities = np.zeros(
        (numRuns, len(Tomographys[0][0].stabilityHistory)), dtype=np.double
    )
    for i in range(numRuns):
        [Tomo_Object, Fidelity_with_Original, Original_Purity, Total_Time] = (
            Tomographys[i]
        )
        try:
            ...
            plt.plot(
                np.arange(1, 1 + numIterPriors),
                np.log10(stabilities[i, 1 : (numIterPriors + 1)]),
                ",-m",
                alpha=0.25,
            )
            plt.plot(
                np.arange(
                    numIterPriors,
                    numIterPriors + len(stabilities[i, numIterPriors:numIterTotal]),
                ),
    ...
    print(
        "Avg num from Prior : "
        + str(np.average(numPriorArray))
        + ", sd : "
        + str(np.std(numPriorArray))
    )
    print(
        "Avg num from Poste : "
        + str(np.average(numPosteArray))
        + ", sd : "
        + str(np.std(numPosteArray))
    )
    print(
        "Avg num from Total : "
        + str(np.average(numTotalArray))
        + ", sd : "
        + str(np.std(numTotalArray))
    )
    print(
        "Avg Time : "
        + str(np.average(numTimeArray))
        + ", sd : "
        + str(np.std(numTimeArray))
    )
