...
total_data <- read_csv(file = here("Tests/Results/simulatedTomographyData.csv"))
```
### Head of data:
```{r, echo = FALSE}
# if time is over 30 minutes for 1qubit then error
total_data <- total_data %>%
              mutate(errorOccurred = case_when(total_time>=60*30 & nqubits == 1 ~ TRUE,
                                               TRUE ~ errorOccurred))

head(total_data)

cat(nrow(total_data)," Data Samples\n",
    nrow(filter(total_data,errorOccurred==1))," Errors",
    sep="")
```
### Summary:
```{r}
...
total_data <- total_data %>%
                filter(errorOccurred==0)
summary_stats
```

```{r, echo = FALSE}
df_mle_1 = total_data %>%   filter(method == "MLE",nqubits==1,errorOccurred==0)
df_bme_1 = total_data %>%   filter(method == "BME",nqubits==1,errorOccurred==0)
df_mle_2 = total_data %>%   filter(method == "MLE",nqubits==2,errorOccurred==0)
df_bme_2 = total_data %>%   filter(method == "BME",nqubits==2,errorOccurred==0)
df_mle_3 = total_data %>%   filter(method == "MLE",nqubits==3,errorOccurred==0)
df_bme_3 = total_data %>%   filter(method == "BME",nqubits==3,errorOccurred==0)
```
